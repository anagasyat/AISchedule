...
async function getSemestersId(preUrl, courseTableCon) {
  ...
  let data = 'dataType=semester'

  let currentYear = new Date().getFullYear()
  
  let semestersText = await request("post",data,xqurl,'加载学期中')
  let semesters = semestersText.split('\n')

  let count = 0
  let semesterIndexTag = 0
  let selectList = []

  for (line of semesters) {
    if (line.search(currentYear) != -1) {
      let tempDom = textToDom(line)
       let value =  tempDom.getElementsByTagName('option')[0].value
       let key = tempDom.getElementsByTagName('option')[0].innerText
        let xnxq = key.split("-")
        selectList.push(
          semesterIndexTag++ +
            ':' +
...
async function scheduleHtmlProvider(
  ...
  if (message.length != 0) {
    courseArr.length = 0
    let errText = `
      遇到错误，请凭此页面截图，加群:496486285,找开发者{啊也}进行反馈
      错误：${message},
      url:${preUrl}
      `
    AIScheduleAlert({
